...
function smoothDampVec2(
  ...
  const targetAdjusted = current.clone().sub(change)
  const temp = currentVelocity.clone().addScaledVector(change, omega).multiplyScalar(deltaTime)
  currentVelocity.sub(temp.clone().multiplyScalar(omega))
  currentVelocity.multiplyScalar(exp)
  out.copy(targetAdjusted.clone().add(change.add(temp).multiplyScalar(exp)))
...
function smoothDampFloat(
  ...
  let out = targetAdjusted + (change + temp) * exp
...
export interface GridScanJSCSSProps extends React.HTMLAttributes<HTMLDivElement> {
  sensitivity?: number
...
export function GridScanJSCSS({
  ...
  return (
    <div
      ref={containerRef}
      className={`gridscan ${className ?? ''}`.trim()}
      style={style}
      {...rest}
    />
  )
